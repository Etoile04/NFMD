...
def run_pipeline(mode: str, source_dir: str, alias_map: str, run_id: str = None):
    ...
    if not records:
        print("  No records extracted. Aborting.")
        meta["status"] = "empty"
    ...
    if fatal_count > 0:
        print("\n  [FATAL] Aborting due to fatal issues.")
        meta["status"] = "fatal"
    ...
    print(f"  Materials resolved: {resolved}")
    print(f"  Materials unresolved: {unresolved}")
    ...
    if mode == "dry-run":
        ...
        print(f"\n  Inserted: {load_stats['parameters_inserted']}")
        print(f"  Updated: {load_stats['parameters_updated']}")
        print(f"  Skipped: {load_stats['parameters_skipped']}")
        print(f"  Errored: {load_stats['parameters_errored']}")
    ...
    if unresolved > 0:
        ...
        print(f"\nUnmapped materials ({len(unmapped)}):")
        for mat, count in sorted(unmapped.items(), key=lambda x: -x[1])[:20]:
            print(f"  {mat}: {count}")

    print(f"\nDone in {elapsed:.1f}s — artifacts in {run_dir}")
